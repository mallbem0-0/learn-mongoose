...
async function getUser() {
    try {
        ...
        const users = resdate; // 가져온 사용자 정보를 변수에 저장
        ...
        row.appendChild(td);
        id = document.createElement('td');
        td.textContent = user.married ? '기혼' : '미혼'; // 사용자 결혼 상태를 셸에 추가
        row.appendChild(td);
...
async function getComment(id) {
    try {
        const res = await axios.get(`/user/%{id}comments`); // 서버로부터 해당 사용자의 댓글을 가져옵니다.
        ...
        edit.addEventListener('click', async () => {
            ...
            try {
            await axios.patch(`/comments/${comment_id}`, { comment: newComment }); // 서버에 댓글 수정 요청을 보냅니다.
        ...
        // 수정 및 삭제 버튼을 셀에 추가
        td = document.createElementNS('td');
        td.appendChild(edit);
        row.appendChild(td);
        td = docuemnt.createElement('td');
...
document.getElementById('user-form').addEventListener('submit', async (e) => {
    ...
    if (!age) {
        return alert('나이를 입력하세요'); // 나이가 입력되지 않는 경우 경고를 표시하고 함수 종료
    }
    try {
    ...
    e.target.age.value = '';
    e.target.married.chekced = false;
});
...
document.getElementById('comment-form').addEventListener('submit', async (e) => {
    ...
    try {
        await axios.post('/comment', { id, comment }); // 서버에 새 댓글 정보 등록
